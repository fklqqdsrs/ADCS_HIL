...
extern void service_adcs_register(csp_conn_t *conn, csp_packet_t *packet);
...
CSP_DEFINE_TASK(task_server) {
	...
	//			service_react_v(conn, packet);
				break;

			case PORT_REGISTER:
				service_adcs_register(conn, packet);
				break;
...
CSP_DEFINE_TASK(task_client) {
	...
	while (1) {
//csp_ps(12, 1000);csp_ps(16, 1000);csp_ps(21, 1000);
		csp_sleep_ms(1000);
...
int test_csp_server_task(unsigned int task_stack_size,
		unsigned int task_priority) {
	int ret = csp_thread_create(task_server, "SERVER", task_stack_size, NULL,
			task_priority, NULL);
	if (ret != 0) {
...
int test_csp_client_task(unsigned int task_stack_size,
		unsigned int task_priority) {
	int ret = csp_thread_create(task_client, "CLIENT", task_stack_size, NULL,
			task_priority, NULL);
	if (ret != 0) {
...
int test_csp_server_client_task(unsigned int task_stack_size,
		unsigned int task_priority) {
	int ret1 = csp_thread_create(task_server, "SERVER", task_stack_size, NULL,
			task_priority, NULL);
	if (ret1 != 0) {
		csp_log_error("Failed to start server task, error: %d", ret1);
		return ret1;
	}
	int ret2 = csp_thread_create(task_client, "CLIENT", task_stack_size, NULL,
			task_priority, NULL);
	if (ret2 != 0) {
...
//    csp_debug_level_t debud_level = CSP_PROTOCOL ;
//    for (csp_debug_level_t i = static_cast<csp_debug_level_t>(0); i <= CSP_LOCK;i = static_cast<csp_debug_level_t>(static_cast<int>(i)+1)){
//    	csp_debug_set_level(i, (i <= debud_level)? true:false);
//    }
    	...
    	csp_driver_can_init(0, CSP_CAN_PROMISC, &default_iface);
    	csp_driver_can_init1(1, CSP_CAN_PROMISC, &default_iface);
    	//rtable = "19 LOOP, 0/0 CAN";
    	rtable = "0/0 CAN, 16/5 CAN_A, 17/5 CAN_A";
    	//rtable = "0/0 CAN";
    	//rtable = "0/0 CAN_A";
    	if (rtable) {
    		error = csp_rtable_load(rtable);
    		if (error < 1) {
    			csp_log_error("csp_rtable_load(%s) failed, error: %d", rtable,
    					error);
    			exit(1);
    		}
...
//    	csp_conn_print_table();
  ...
  {
    /* USER CODE END WHILE */
	  int res = HAL_CAN_GetRxFifoFillLevel(&hcan2, CAN_RX_FIFO0);
	  int re3 = HAL_CAN_GetRxFifoFillLevel(&hcan2, CAN_RX_FIFO1);
