									 ...
									 TctlmCubeCommonBaseBootloader5_Errors *errors)
{
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		if (result == CUBEOBC_ERROR_OK)
		{
			TctlmCubeCommonBaseBootloader5_FileInfo fileInfo;
			Boolean done = FALSE;
			Boolean found = FALSE;
			...
			do
			{
				...
				if (result == CUBEOBC_ERROR_OK)
				{
					found = (fileInfo.program == TCTLM_COMMON_FRAMEWORK_ENUMS__PROGRAM_TYPE_BOOTLOADER);
	...
	TctlmCubeCommonBaseBootloader5_OptionBytes optionBytes;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		if (result == CUBEOBC_ERROR_OK)
		{
			TctlmCubeCommonBaseBootloader5_OptionBytes optionBytesConfirm;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		result = tctlmCubeCommonBaseBootloader5_setCommitOptionBytes(endpoint, 777777777u);
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		...
		if (result == CUBEOBC_ERROR_OK)
		{
			...
			result = tctlmCubeCommonBaseBootloader5_getMemMap(endpoint, &memrmp);

			if (result == CUBEOBC_ERROR_OK)
			{
				// Highly unlikely scenario
				if (optionBytesConfirm.bfb2 != memrmp)
				{
					result = CUBEOBC_ERROR_UNKNOWN;
	...
	if (result == CUBEOBC_ERROR_OK)
	{
		TctlmCubeCommonBaseBootloader5_Identification id;
