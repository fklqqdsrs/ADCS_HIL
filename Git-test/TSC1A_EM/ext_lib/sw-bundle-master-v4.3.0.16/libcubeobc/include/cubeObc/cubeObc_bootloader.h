...
/**
 * @brief Upgrade the bootloader with a CubeSpace (-bin-upgrade.cs) file
 *
 * @note N.B! This function is implemented as a reference and is not intended to be used in orbit.
 * Although it works, the user is encouraged to implement their own routine so that they may
 * interface with their OBC systems to manage critical sections, power supply, logging, etc.
 *
 * @param[in]	endpoint	Comms endpoint
 * @param[in]	size		The size of the CubeSpace file
 * @param[in]	userData	User specific data
 * @param[in]	vMajor		The expected major version of the bootloader following the upgrade
 * @param[in]	vMinor		The expected minor version of the bootloader following the upgrade
 * @param[out]	errors		The last requested Errors telemetry for debugging if return is not CUBEOBC_ERROR_OK.
 *
 * @return CUBEOBC_ERROR_OK if successful
 */
