...
private:
	...
	uint8_t _len;
	uint8_t _type;
	uint8_t _is_init,_isupdated=0,_need_flash=0;
...
public:
	...
	void init(uint8_t *header, uint8_t len, uint8_t type=0,uint8_t need_flash=0)
	{
		_header = header;
		_len = len;
		_type = type;
	...
	void set_bytes(uint8_t *bytes, uint8_t len,bool endian = LittleEndian)
	{
		if (endian == BigEndian)
		{
			for (int i = 0; i < len; i++)
