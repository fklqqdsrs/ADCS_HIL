#ifndef _CARBONCAN_h_
#define _CARBONCAN_h_
...
//
#ifdef NEWFDCAN
	CarbonCAN(FDCAN_HandleTypeDef *can, uint8_t self_id,
	uint32_t RxFifo = FDCAN_RX_FIFO0) : newPortFDCAN(can,self_id,
	FDCAN_RX_FIFO0)
#else
	CarbonCAN(CAN_HandleTypeDef *can, uint8_t self_id,
	uint32_t RxFifo = CAN_RX_FIFO0) : newPortCAN(can,self_id,
	CAN_RX_FIFO0)
#endif
//
	{
...
//
	...
	{
		*_idbuff = encode_id(ack_type, reg_id, this->get_self_id(), dest_addr);
		set_dlc(0);
	...
	{
		decode_id(*get_extID<_RxHeadType_>(&header)); /*decode 29 bit id*/
		decode_dlc(&header);
		if (_rx_buffer.dest_addr != this->get_self_id())
		{
			_success = 0;
			return 6;
		}
	...
	uint8_t request_data(uint8_t dest_addr, uint8_t reg_id)
	{
		...
		*_idbuff = encode_id(TLM_req, reg_id, this->get_self_id(), dest_addr);
		set_dlc(0);
		_success = 1;
	...
	{
		*_idbuff = encode_id(TC_req, reg_id, this->get_self_id(), dest_addr);
	...
	{
		*_idbuff = encode_id(TC_EXT, reg_id, this->get_self_id(), dest_addr);
		uint8_t ext_r = len/7;
		uint8_t mod = len % 7;
		if(mod >= 1){
			ext_r += 1;
