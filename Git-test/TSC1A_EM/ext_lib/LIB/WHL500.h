...
//	uint8_t RxMsg[8];
	...
	uint8_t WHLID,whlnum;
	uint8_t prior,src_id,dest_id,src_port,dest_port,vartype;
	uint16_t CFP,Param  ;
	uint32_t speed;
	uint32_t VAL;
	float   fVAL;
...
public:
	...
	WHL500(newPortCAN *can,uint8_t _source_id,uint8_t num)
	{
		WHLCAN = can;
		src_id = _source_id;
		whlnum = num;
		whl500_reg_setup(whlnum);
	...
	uint8_t RxMessage(){
		...
		if (deviceCheck() == true)
		{
			if (newMessageCheck() == true)
			{
				...
				memcpy(RxBuffer+RxIndex,RxMsg,WHLCAN->get_rx_head()->DLC);
				RxIndex += WHLCAN->get_rx_head()->DLC;
//			if (RxIndex >= 17)
//			{
//				getValue();
//			}
	...
	void RxMessage(uint8_t *msg,uint8_t len){
...
//			WHLCAN->readBytes(WHLCAN->get_rx_head(), RxMsg);
		 ...
		 */
			...
			if (newMessageCheck() == true)
			{
				...
				memcpy(RxBuffer+RxIndex,msg,len);
				RxIndex += len;
...
//		void* value = (void*)& fVAL;
	...
	void getValue()
	{
	  if(RxBuffer[6] == 0x06)
	  {
	    ...
	    switch(vartype) {
	        case PARAM_TYPE_INT8:
	        	whl500_reg[whlnum][Param].set_bytes(RxBuffer+14, 1,BigEndian);
	            break;
	        case PARAM_TYPE_INT16:
	        	whl500_reg[whlnum][Param].set_bytes(RxBuffer+14, 2,BigEndian);
	            break;
	        case PARAM_TYPE_INT32:
	        	whl500_reg[whlnum][Param].set_bytes(RxBuffer+14, 4,BigEndian);
	            break;
	        case PARAM_TYPE_INT64:
	        	whl500_reg[whlnum][Param].set_bytes(RxBuffer+14, 8,BigEndian);
	        	break;
	        case PARAM_TYPE_UINT16:
	        	whl500_reg[whlnum][Param].set_bytes(RxBuffer+14, 2,BigEndian);
	        	break;
	        ...
	        case PARAM_TYPE_FLOAT64:
	        	...
	        	break;
	        default:
	        	whl500_reg[whlnum][Param].set_bytes(RxBuffer+13, 1,BigEndian);
	        	break;
	        Param = 0;
	    }
	  }
...
//	}
	...
	uint8_t deviceCheck()
	{
		//CANRxIdDecode(ext_id); /*decode 29 bit id*/
