...
#include <include/csp/interfaces/csp_if_kiss.h>
...
#include <include/csp/arch/csp_malloc.h>
...
static int kiss_driver_tx(void *driver_data, const unsigned char * data, size_t data_length) {
	...
	if (csp_radio_write(ctx->fd, data, data_length) == (int) data_length) {
		return CSP_ERR_NONE;
	}
	return CSP_ERR_TX;
}
...
int csp_radio_open_and_add_kiss_interface(const csp_radio_conf_t *conf, const char * ifname, csp_iface_t ** return_iface) {

	if (ifname == NULL) {
		ifname = CSP_IF_RADIO_DEFAULT_NAME;
	}

	csp_log_info("INIT %s: device: [%s], bitrate: %lu",
	...
	if (res == CSP_ERR_NONE) {
		res = csp_radio_open(conf, kiss_driver_rx, ctx, &ctx->fd);
	}
