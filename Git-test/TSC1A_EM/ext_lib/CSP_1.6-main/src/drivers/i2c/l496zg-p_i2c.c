...
#include <errno.h>
#include <fcntl.h>
#include <src/drivers/i2c/l496zg-p_i2c.h>
...
osThreadId_t i2c_rx_tHandle;
const osThreadAttr_t i2c_rx_t_attributes = {
  .name = "i2c_rx_t",
...
CSP_DEFINE_TASK(Master_i2c_rx_task);
#else
#endif
...
uint8_t get_register_i2c(uint32_t address_slave) {
	HAL_StatusTypeDef status = HAL_I2C_Master_Receive(&CONFIG_CSP_I2C,
			address_slave << 1, status_buffer_slave, HEADER_SIZE, 1000);
	while (HAL_I2C_GetState(&CONFIG_CSP_I2C) != HAL_I2C_STATE_READY)
		;
	if (status != HAL_OK) {
		return NONE_ERROR;
	}
...
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef *hi2c) {
	...
	if (errorcode == 4) {
		if (global_i2c_ctx->mode == I2C_MODE_SLAVE)
			slave_i2c_rx_callback();
	}
	if (errorcode == 1) {
		HAL_I2C_DeInit(hi2c);
...
void Master_i2c_rx_task(void const *argument) {
	...
	while (1) {
		...
		if (status == SLAVE_SEND) {
			uint8_t length_rx = status_buffer_slave[1];
			HAL_I2C_Master_Receive_IT(&CONFIG_CSP_I2C,
					ctx->slaveList.addresses[0] << 1, i2c_RxData, length_rx);
		}
	}
}
#elif  OS_VERSION == CSP_THREAD
CSP_DEFINE_TASK(Master_i2c_rx_task) {
    i2c_context_t *ctx = (i2c_context_t*)param;
    while (1) {
        osDelay(50);
        status_slave status = get_register_i2c(ctx->slaveList.addresses[0]);
        if (status == SLAVE_SEND) {
            uint8_t length_rx = status_buffer_slave[1];
            HAL_I2C_Master_Receive_IT(&CONFIG_CSP_I2C, ctx->slaveList.addresses[0] << 1,i2c_RxData, length_rx);
        }
    }
}
...
int csp_i2c_tx_master(void *driver_data, csp_i2c_frame_t *frame) {
	...
	HAL_I2C_Master_Transmit_IT(&CONFIG_CSP_I2C,
			ctx->slaveList.addresses[0] << 1, frame->data, data_length);
...
//	ctx->iface.netmask = netmask;
	...
	if (mode == I2C_MODE_MASTER) {
		...
		i2c_rx_tHandle = osThreadCreate(osThread(i2c_rx_t), ctx);
		   if (i2c_rx_tHandle == NULL) {
				        printf("Failed to create thread : Master_i2c_rx_task : CMSIS_V1\n");
				    }
#elif OS_VERSION == CMSIS_V2
		 i2c_rx_tHandle = osThreadNew(Master_i2c_rx_task, ctx, &i2c_rx_t_attributes);
		    if (i2c_rx_tHandle == NULL) {
		        printf("Failed to create thread : Master_i2c_rx_task : CMSIS_V2\n");
		    }
#elif OS_VERSION == CSP_THREAD
		    csp_thread_func_t routine = Master_i2c_rx_task;
		    const char * const thread_name = "i2c_rx_t";
	...
	int result = csp_thread_create(routine, thread_name, stack_size, parameters, priority, &return_handle);
	  if (result != CSP_ERR_NONE) {
		  printf("Failed to create thread : Master_i2c_rx_task : csp thread\n");
 }
#else
#error "Unknown OS_VERSION"
#endif
		initializeSlaveAddressList(ctx);
		addSlaveAddress(ctx, 0x27);
	} else {
		ctx->ifdata.tx_func = csp_i2c_tx_slave;
		if (HAL_I2C_EnableListen_IT(&CONFIG_CSP_I2C) != HAL_OK) {
			printf("HAL_I2C_EnableListen_IT(&CSP_I2C) != HAL_OK\r\n");
			Error_Handler();
		}
	}
	global_i2c_ctx = ctx;
