...
#include <include/csp/arch/csp_clock.h>
#include <include/csp/arch/csp_system.h>

#if (CSP_DEBUG) && (CSP_USE_EXTERNAL_DEBUG == 0)

#include CONFIG_STM32_HAL
extern UART_HandleTypeDef CONFIG_CSP_PRINT;

int _write(int fd, char* ptr, int len) {
    HAL_UART_Transmit(&CONFIG_CSP_PRINT, (uint8_t *) ptr, len, HAL_MAX_DELAY);
    return len;
}
...
void do_csp_debug(csp_debug_level_t level, const char *format, ...) {
	...
	if (csp_debug_hook_func) {
		...
		csp_sys_set_color(color);
                ...
                printf("%lu.%06lu ", ts.tv_sec, ts.tv_nsec / 1000UL);
