...
#ifdef TSC_USING_POSIX_DELAY
#include <delay.h>
#endif
#if ENABLE_TIME_RTC || ENABLE_TIME_PPS
...
__attribute__((aligned(4))) static const char *days = "Sun Mon Tue Wed Thu Fri Sat ";
__attribute__((aligned(4))) static const char *months = "Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec ";
 ...
 * @return the operation status, TSC_EOK on successful
 */
static int get_timeval(struct timeval *tv)
{
    ...
    error = time_manager_get_pps_time(&tv->tv_sec);
    ...
    error = time_manager_get_rtc_time(&tv->tv_sec);

    if(!error)
    	return error;
#endif
#if !ENABLE_TIME_RTC && !ENABLE_TIME_PPS
//    LOG_W(_WARNING_NO_TIME_SOURCE);
    return -TSC_ENOSYS;
#else /* NO_TIME_SOURCE */
    return error;
#endif
...
#if ENABLE_TIME_PPS
	error |= time_manager_set_pps_time(tv->tv_sec);
#endif
#if ENABLE_TIME_RTC
	error |= time_manager_set_rtc_time(tv->tv_sec);
#endif
#if !ENABLE_TIME_RTC && !ENABLE_TIME_PPS
//    LOG_W(_WARNING_NO_TIME_SOURCE);
	return -TSC_ENOSYS;
...
char* asctime_r(const struct tm *t, char *buf)
{
    ...
    if ((int)tsc_strlen(days) <= (t->tm_wday << 2) || (int)tsc_strlen(months) <= (t->tm_mon << 2))
    {
 ...
 *
 */
__weak time_t time(time_t *t)
{
    struct timeval now;

    if(get_timeval(&now) == TSC_EOK)
    {
        if (t)
        {
            *t = now.tv_sec;
...
int gettimeofday(struct timeval *tv, struct timezone *tz)
{
    ...
    if (tv != NULL && get_timeval(tv) == TSC_EOK)
    {
        return 0;
    }
    else
...
int settimeofday(const struct timeval *tv, const struct timezone *tz)
{
    ...
    if (tv != NULL
        && tv->tv_usec >= 0
        && set_timeval((struct timeval *)tv) == TSC_EOK)
    {
        return 0;
    }
    else
...
/* timezone */
#ifndef TSC_LIBC_DEFAULT_TIMEZONE
#define TSC_LIBC_DEFAULT_TIMEZONE    0
#endif

static volatile int8_t _current_timezone = TSC_LIBC_DEFAULT_TIMEZONE;
