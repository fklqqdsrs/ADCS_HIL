...
size_t hash_function(unsigned int device_id, unsigned int register_id, unsigned int sub_register) {
	if (csp_register_map == NULL || csp_register_map->size == 0) {
		DEBUG_PRINT_REG("RegisterMap is not initialized or size is zero\n");
		return 0;
	}
...
map_node_t* find_register_node(unsigned int device_id, unsigned int register_id, unsigned int sub_register) {
	if (csp_register_map == NULL) {
		DEBUG_PRINT_REG("RegisterMap is not initialized\\n");
		return NULL;
	}
...
int8_t initializeRegister(size_t size) {
	if (size == 0) {
		DEBUG_PRINT_REG("Invalid size for RegisterMap initialization\n");
		return -1;
	}

	csp_register_map = (RegisterMap*) malloc(sizeof(RegisterMap));
	if (csp_register_map == NULL) {
		DEBUG_PRINT_REG("Failed to create RegisterMap\n");
		return -2;
	}

	csp_register_map->buckets = (map_node_t**) calloc(size, sizeof(map_node_t*));
	if (csp_register_map->buckets == NULL) {
		DEBUG_PRINT_REG("Failed to allocate memory for buckets\n");
		free(csp_register_map);
		csp_register_map = NULL;
		return -3;
	}
	...
	if (csp_mutex_create(&register_mutex) != CSP_MUTEX_OK) {
		...
		DEBUG_PRINT_REG("Failed to create mutex.\n");
		free(csp_register_map->buckets);
		free(csp_register_map);
		csp_register_map = NULL;
		return -4;
	}
...
bool add_register(unsigned int device_id, unsigned int register_id, unsigned int sub_register, RegisterType type, void *value, AccessMode access_mode) {
	...
	size_t value_size = get_value_size(type);
	if (value_size == 0) {
		DEBUG_PRINT_REG("Invalid register type\n");
		return false;
	}

	csp_mutex_lock(&register_mutex, CSP_MAX_TIMEOUT);

	if (find_register_node(device_id, register_id, sub_register) != NULL) {
		DEBUG_PRINT_REG("Register already exists.\n");
		csp_mutex_unlock(&register_mutex);
		return false;
	}

	size_t key_size = sizeof(RegisterKey);
	size_t node_size = sizeof(map_node_t);
	uintptr_t base_ptr = (uintptr_t) malloc(node_size + key_size + value_size);
	if (!base_ptr) {
		csp_mutex_unlock(&register_mutex);
		return false;
	}

	map_node_t *new_node = (map_node_t*) base_ptr;
	RegisterKey *key = (RegisterKey*) (base_ptr + node_size);
	void *node_value = (void*) (base_ptr + node_size + key_size);

	key->device_id = device_id;
	key->register_id = register_id & 0xFF;
	key->sub_register = sub_register & 0xFFFF;
...
bool set_register(unsigned int device_id, unsigned int register_id, unsigned int sub_register, void *value) {
	csp_mutex_lock(&register_mutex, CSP_MAX_TIMEOUT);
	map_node_t *node = find_register_node(device_id, register_id, sub_register);
	if (node) {
		if (node->access_mode == READ_ONLY) {
			DEBUG_PRINT_REG("Cannot set value. Register is read-only.\n");
			csp_mutex_unlock(&register_mutex);
			return false;
			...
			DEBUG_PRINT_REG("Unknown register type.\n");
			csp_mutex_unlock(&register_mutex);
			return false;
		}
	} else {
		DEBUG_PRINT_REG("Register not found. Please add it first.\n");
		csp_mutex_unlock(&register_mutex);
		return false;
	}
...
int8_t get_register(unsigned int device_id, unsigned int register_id, unsigned int sub_register, void *outValue) {
	...
	if (!node) {
		DEBUG_PRINT_REG("Register not found.\n");
		csp_mutex_unlock(&register_mutex);
		return -3;
	}

	if (device_id != csp_get_address() && node->access_mode == WRITE_ONLY) {
		DEBUG_PRINT_REG("Cannot read value. Register is write-only.\n");
		csp_mutex_unlock(&register_mutex);
		return -1;
	}
...
bool remove_register(unsigned int device_id, unsigned int register_id, unsigned int sub_register) {
	csp_mutex_lock(&register_mutex, CSP_MAX_TIMEOUT);
	...
	while (node) {
		RegisterKey *key = (RegisterKey*) node->key;
		if (key->device_id == device_id && key->register_id == (uint8_t) register_id && key->sub_register == (uint16_t) sub_register) {
			if (prev) {
				prev->next = node->next;
			} else {
				csp_register_map->buckets[hash] = node->next;
			}
			free(node);
			csp_mutex_unlock(&register_mutex);
			return true;
