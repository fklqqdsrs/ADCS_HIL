...
void ringbuffer_init(struct ringbuffer *rb, uint8_t *pool, int16_t size)
{
    ...
    rb->buffer_ptr = pool;
    rb->buffer_size = TSC_ALIGN_DOWN(size, TSC_ALIGN_SIZE);
}
                            ...
                            uint16_t           length)
{
    ...
    if (rb->buffer_size - rb->write_index > length)
    {
        ...
        tsc_memcpy(&rb->buffer_ptr[rb->write_index], ptr, length);
    ...
    tsc_memcpy(&rb->buffer_ptr[rb->write_index],
           &ptr[0],
           rb->buffer_size - rb->write_index);
    tsc_memcpy(&rb->buffer_ptr[0],
                            ...
                            uint16_t           length)
{
    ...
    {
        /* copy all of data */
        tsc_memcpy(ptr, &rb->buffer_ptr[rb->read_index], length);
        /* this should not cause overflow because there is enough space for
         * length of data in current mirror */
    ...
    tsc_memcpy(&ptr[rb->buffer_size - rb->read_index],
           &rb->buffer_ptr[0],
 ...
 */
size_t ringbuffer_peak(struct ringbuffer *rb, uint8_t **ptr)
{
    size_t size;
...
struct ringbuffer *ringbuffer_create(uint16_t size)
{
    ...
    size = TSC_ALIGN_DOWN(size, TSC_ALIGN_SIZE);

    rb = (struct ringbuffer *)tsc_malloc(sizeof(struct ringbuffer));
    if (rb == NULL)
        goto exit;

    pool = (uint8_t *)tsc_malloc(size);
    if (pool == NULL)
    {
        tsc_free(rb);
        rb = NULL;
