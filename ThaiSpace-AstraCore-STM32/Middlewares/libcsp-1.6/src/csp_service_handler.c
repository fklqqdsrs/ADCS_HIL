...
void csp_service_handler(csp_conn_t * conn, csp_packet_t * packet) {
	...
	case CSP_INTERFACE: {
		/* Sanity check on request */
		if ((packet->length != 1) || (packet->data[0] != 0x66)) {
			/* Sanity check failed */
			csp_buffer_free(packet);
			/* Clear the packet, it has been freed */
			packet = NULL;
		...
		int if_num = 1;
		do {
			int iflen = csp_iflist_sprintf(iflist, 256, if_num);
			...
			while (i < iflen) {
				...
				/* Calculate length, either full MTU or the remainder */
				packet->length =
						(iflen - i > CSP_RPS_MTU) ? CSP_RPS_MTU : (iflen - i);

				/* Send out the data */
				memcpy(packet->data, &iflist[i], packet->length);
				i += packet->length;
				if (!csp_send(conn, packet, 0))
					csp_buffer_free(packet);
